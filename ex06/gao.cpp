...
unordered_map<LL, bool> reduced_values;
unordered_map<LL, int>  cal_value_times;
void gao_item_value_reduce(char * dim_items_txt){
    ...
    vector<PLL> vp;
    for(unordered_map<LL, int>::iterator it = cal_value_times.begin(); it != cal_value_times.end(); it++){
        vp.push_back(PLL(it->second, it->first));
    }
    sort(vp.begin(), vp.end());
    reverse(vp.begin(), vp.end());

    for(int i = 0; i < REDUCE_TOP_NUMBER; i++){
        cerr<<"appear times:\t"<<vp[i].first<<"\tvalue:\t"<<vp[i].second<<endl;
        reduced_values[vp[i].second] = true;
    }
    //exit(0);
}
...
void gao_TF_IDF(char * dim_items_txt){
    ...
    while(item_values_file >> item >> cat >> values_string){
        ...
        while(ss >> val){
            ...
            // if val in reduced_values, ignore
            if(reduced_values.find(val) != reduced_values.end()){
                continue;
            }
